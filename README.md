...
This repository contains R code and data for analyzing tweets about ASLO 2018. The analysis is based around the `rtweet` package, and the code is largely based on work `rtweets` author [Mike Kearney](https://github.com/mkearney) did for other conferences. My thanks to him for letting me use it.
...
## search for up to 100,000 tweets mentioning ASLO 2018
rt <- search_tweets(
...
# rt <- readRDS(file.path("data", "search.rds"))
```

From there, it's easy to plot the number of tweets about ASLO over time.
...
rt %>%
  filter(as.Date(created_at) > as.Date("2018-06-10")) %>%
...
rt %>%
  ...
  theme(legend.position = "none",
    ...
    labs(x = NULL, y = NULL,
    title = "Sentiment (valence) of ASLO 2018 tweets over time",
...
## cleanup graph by removing rare screen_names
common_screen_names <- group_by(rt, screen_name) %>% 
  dplyr::summarise(n()) %>% 
  dplyr::select(screen_name, count  = `n()`) %>%
  dplyr::filter(count >= 7 | screen_name == "__jsta")

rt <- dplyr::filter(rt, screen_name %in% common_screen_names$screen_name)  
...
as_tbl_graph(g) %>% 
  ...
  theme(plot.title = element_text(size = rel(1.5), face = "bold", margin = margin(3, 1, 10, 1)),
      plot.subtitle = element_text(size = rel(1.2)),
      plot.caption = element_text(colour = "#444444")) +
  labs(title = "Connections between Twitter users during ASLO 2018",
    caption = "\n\nSource: Data gathered via Twitter's standard `search/tweets` API using rtweet\n Code provided by Mike Kearney (@kearneymw)",
    x = NULL, y = NULL)
```
